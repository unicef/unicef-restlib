...
def test_creation(author):
    serializer = BookSeparatedSerializer(
        data={
            "name": "Scary Tales",
            "sku_number": "123",
            "author": author.pk,
...
def test_validation():
    ...
    assert serializer.errors == {"author": ['Invalid pk "404" - object does not exist.']}
...
def test_validate_empty_required():
    serializer = BookSeparatedWriteSerializer(
        data={
            "name": "Scary Tales",
            "sku_number": "123",
        }
...
def test_validate_none_not_required():
    ...
    serializer = BookSeparatedSerializer(data={"name": "Scary Tales", "sku_number": "123", "author": None})
    assert serializer.is_valid()


def test_validate_none_required():
    serializer = BookSeparatedWriteSerializer(data={"name": "Scary Tales", "sku_number": "123", "author": None})
    assert not serializer.is_valid()
...
def test_dynamic_choices_field():
    serializer = BookSerializer(data={"name": "Scary Tales", "genre": "western", "sku_number": "123"})
...
def test_dynamic_choices_field_not_choices(user):
    serializer = ReviewMetaSerializer(data={"user": user.pk, "rating": 1, "status": "new"})
...
def test_dynamic_choices_field_invalid():
    serializer = BookSerializer(data={"name": "Scary Tales", "genre": "wrong", "sku_number": "123"})
...
def test_model_choice_field_valid_serializer():
    ...
    valid_serializer = ImageFileTypeSerializer(data={"file_type": file_type.pk})
    assert valid_serializer.is_valid()


def test_model_choice_field_invalid_serializer():
    file_type = FileTypeFactory(code="wrong")
    invalid_serializer = ImageFileTypeSerializer(data={"file_type": file_type.pk})
